...
function Cards() {
    ...
    useEffect(() => {
        // API에서 카테고리 데이터 가져오기
        fetch('https://port-0-gowo-12fhqa2llodwi7b3.sel5.cloudtype.app/place/category')
    ...
    useEffect(() => {
        if (selectedCategory !== "전체") {
          // API에서 데이터 가져오기
          fetch(`https://port-0-gowo-12fhqa2llodwi7b3.sel5.cloudtype.app/place?place=${selectedCategory}`)
            .then(response => {
              if (!response.ok) {
...
// '전체' 카테고리를 선택했을 때 모든 데이터를 가져오기
fetch('https://port-0-gowo-12fhqa2llodwi7b3.sel5.cloudtype.app/place')
.then(response => response.json())
.then(apiData => {
