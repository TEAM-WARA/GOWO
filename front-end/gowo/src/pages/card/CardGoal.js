...
function Cards() {
    ...
    useEffect(() => {
        // API에서 카테고리 데이터 가져오기
        fetch('https://port-0-gowo-12fhqa2llodwi7b3.sel5.cloudtype.app/health/category')
          .then(response => response.json())
          .then(categoryData => {
            setCategoryData(categoryData.category);
            setSelectedCategory(categoryData.category[0]);
          })
          .catch(error => {
    ...
    useEffect(() => {
        if (selectedCategory !== "전체") {
          // API에서 데이터 가져오기
          fetch(`https://port-0-gowo-12fhqa2llodwi7b3.sel5.cloudtype.app/health?health=${selectedCategory}`)
            .then(response => {
              if (!response.ok) {
            ...
            fetch(`https://port-0-gowo-12fhqa2llodwi7b3.sel5.cloudtype.app/health`)
            .then(response => {
              if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
              }
